...
const linkClasses = computed(() => {
  ...
  return props.to
    ? `${baseClasses} transition-opacity cursor-pointer`
    : baseClasses;
});
...
<template>
  <component
    ...
    :class="[
      'logo-container',
      linkClasses,
      { 'group-hover-enabled': props.animateOnGroupHover },
    ]"
  >
    <!-- Icon SVG -->
    <div
      :class="['logo-icon', sizeClasses.icon]"
      :style="{ '--animation-scale': animationScale }"
    >
      <svg
      ...
      >
        <defs>
          ...
          >
            <stop
              offset="0"
              style="stop-opacity: 1; stop-color: var(--color-primary)"
            />
            <stop
              offset="1"
              style="stop-opacity: 1; stop-color: var(--color-secondary)"
            />
          </linearGradient>
        </defs>
        <path
...
:global(.group:hover .logo-container.group-hover-enabled .logo-icon) {

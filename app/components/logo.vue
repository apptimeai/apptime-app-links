...
const props = withDefaults(defineProps<Props>(), {
  ...
  iconFill: 'var(--color-primary)',
});

// Size mappings for sm, md, lg
const sizeMap = {
  xs: { icon: 'h-4 w-4', text: 'h-[0.875rem]', gap: 'gap-1.5' },
  sm: { icon: 'h-5 w-5', text: 'h-[1rem]', gap: 'gap-2' },
  md: { icon: 'h-6 w-6', text: 'h-[1.125rem]', gap: 'gap-2.5' },
  lg: { icon: 'h-7 w-7', text: 'h-[1.25rem]', gap: 'gap-2.5' },
  xl: { icon: 'h-8 w-8', text: 'h-[1.5rem]', gap: 'gap-3' },
  '2xl': { icon: 'h-10 w-10', text: 'h-[1.75rem]', gap: 'gap-3.5' },
};

const sizeClasses = computed(() => sizeMap[props.size]);

const linkClasses = computed(() => {
  const baseClasses = `flex flex-row items-center ${sizeClasses.value.gap}`;
  return props.to
    ? `${baseClasses} transition-opacity hover:opacity-90 cursor-pointer`
    : baseClasses;
});
...
<template>
  <component
    :is="wrapperComponent"
    :href="to || undefined"
    :aria-label="to ? 'Logo Apptime' : undefined"
    :class="['logo-container', linkClasses]"
  >
    <!-- Flat Icon SVG -->
    <div :class="['logo-icon shrink-0', sizeClasses.icon]">
      <svg
    ...
    <!-- Flat Text SVG -->
    <div v-if="!iconOnly" :class="['logo-text shrink-0', sizeClasses.text]">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        xml:space="preserve"
        height="100%"
